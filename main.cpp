...
GLboolean redFlag = true, fanSwitch = false,fanvolom = false,  switchOne = false, switchTwo=false, switchLamp=false, amb1=true, diff1=true, spec1=true, amb2=true, diff2=true, spec2=true, amb3=true, diff3=true, spec3=true;
double windowHeight=800, windowWidth=600;
double eyeX=7.0, eyeY=2.0, eyeZ=15.0, refX = 0, refY=0,refZ=0;
double theta = 180.0, y = 1.36, z = 7.97888;float a = 0.0;
...
void myKeyboardFunc( unsigned char key, int x, int y )
{
    switch ( key )
    {
        ...
        case't':
            ...
            case 'h': //turn on/off fan
            if(fanvolom == false) {
                fanvolom = true; break;
            }
            else{
                fanvolom = false; break;
            }
...
void animate()
{
    ...
    if(fanSwitch == true)
    {
        ...
        a += 0.50;   // fan speed (increase for faster rotation)
        if(a > 360)
            a -= 360;
    }
    if(fanvolom == true)
    {
        ...
        a += 5.0;   // fan speed (increase for faster rotation)
        if(a > 360)
            a -= 360;
    }
